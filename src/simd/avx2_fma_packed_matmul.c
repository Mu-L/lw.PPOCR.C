...
void lw_avx2_fma_packed_matmul_argmax_scores_f32(
    ...
    for (batch = 0u; batch < batch_count; ++batch) {
        uint32_t row;
        for (row = 0u; row + 4u <= rows; row += 4u) {
            /* Per-row running max/index pairs across all panels; one low and
             * one high half per row, never materialized as logits. */
            __m256 maxima[8];
            __m256 maxima_indices[8];
            const __m256 negative_infinity = _mm256_set1_ps(-(float)INFINITY);
            uint32_t current_row;
            uint32_t column_panel;
            int stored = 0;
            for (current_row = 0u; current_row < 8u; ++current_row) {
                maxima[current_row] = negative_infinity;
                maxima_indices[current_row] = _mm256_setzero_ps();
            }
            for (column_panel = 0u; column_panel < column_panels; ++column_panel) {
                __m256 accumulators[8] = {
                    _mm256_setzero_ps(), _mm256_setzero_ps(), _mm256_setzero_ps(),
                ...
                }
                {
                    ...
                    if (valid_columns == LW_PACKED_MATMUL_COLUMN_TILE) {
                        for (current_row = 0u; current_row < 4u; ++current_row) {
                            update_max256(&maxima[current_row * 2u],
                                          &maxima_indices[current_row * 2u],
                                          accumulators[current_row * 2u], column_base,
                                          lane_offsets);
                            update_max256(&maxima[current_row * 2u + 1u],
                                          &maxima_indices[current_row * 2u + 1u],
                                          accumulators[current_row * 2u + 1u],
                                          column_base + 8u, lane_offsets);
                        }
                    } else {
                        /* Partial final panel: reduce the vector state to
                         * scalars, then continue the same strict-greater merge
                         * for the remaining columns. */
                        for (current_row = 0u; current_row < 4u; ++current_row) {
                            const uint64_t result_row =
                                (uint64_t)batch * rows + row + current_row;
                            float values[LW_PACKED_MATMUL_COLUMN_TILE];
                            float best_value = -(float)INFINITY;
                            uint32_t best_index = 0u;
                            uint32_t lane;
                            reduce_max8(maxima[current_row * 2u],
                                        maxima_indices[current_row * 2u],
                                        &best_value, &best_index);
                            reduce_max8(maxima[current_row * 2u + 1u],
                                        maxima_indices[current_row * 2u + 1u],
                                        &best_value, &best_index);
                            _mm256_storeu_ps(values, accumulators[current_row * 2u]);
                            _mm256_storeu_ps(values + 8u, accumulators[current_row * 2u + 1u]);
            ...
            if (!stored) {
                for (current_row = 0u; current_row < 4u; ++current_row) {
                    ...
                    float best_value = -(float)INFINITY;
                    uint32_t best_index = 0u;
                    reduce_max8(maxima[current_row * 2u], maxima_indices[current_row * 2u],
                                &best_value, &best_index);
                    reduce_max8(maxima[current_row * 2u + 1u],
                                maxima_indices[current_row * 2u + 1u],
                                &best_value, &best_index);

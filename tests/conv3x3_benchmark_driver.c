...
static kernel_fn select_kernel(lw_simd_level level, const char** name) {
    if (lw_simd_level_is_avx2(level)) {
        *name = "avx2";
        return lw_avx2_conv3x3_unit_pad1_f32;
    }
    if (lw_simd_level_is_sse2(level)) {
        *name = "sse2";
        return lw_sse2_conv3x3_unit_pad1_f32;
    }
    if (lw_simd_level_is_neon(level)) {
        *name = "neon";
        return lw_neon_conv3x3_unit_pad1_f32;
    }
    *name = "scalar";
    return lw_scalar_conv3x3_unit_pad1_f32;
}

int main(int argc, char** argv) {
    uint32_t ic = 64u, oc = 16u, height = 128u, width = 128u, iterations = 10u;
    ...
    if (argc > 6 || height > (uint32_t)INT32_MAX || width > (uint32_t)INT32_MAX) {
        fprintf(stderr, "usage: conv3x3-benchmark-driver [ic] [oc] [height] [width] [iterations]\n");
        return 2;
    }

    input_count = (uint64_t)ic * height * width;
    weight_count = (uint64_t)oc * ic * 9u;
    output_count = (uint64_t)oc * height * width;
    if (input_count > SIZE_MAX / sizeof(float) ||
        weight_count > SIZE_MAX / sizeof(float) ||
        output_count > SIZE_MAX / sizeof(float)) return 2;

    input = (float*)malloc((size_t)input_count * sizeof(float));
    weights = (float*)malloc((size_t)weight_count * sizeof(float));
    bias = (float*)malloc((size_t)oc * sizeof(float));
    scalar_output = (float*)malloc((size_t)output_count * sizeof(float));
    simd_output = (float*)malloc((size_t)output_count * sizeof(float));
    ...
    output_dims[2] = (int32_t)height; output_dims[3] = (int32_t)width;

    if (!measure(lw_scalar_conv3x3_unit_pad1_f32, input, weights, bias,
    ...
    printf("{\"schema_version\":1,\"input_channels\":%u,\"output_channels\":%u,"
           ...
           ic, oc, height, width, height, width, iterations, backend,

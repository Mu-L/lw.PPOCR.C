...
class ReleaseAssetContractTests(unittest.TestCase):
    ...
    def test_repository_workflow_uses_manifest_and_full_tag_version(self) -> None:
        ...
        self.assertNotIn("pattern: lw.PPOCR.C-*", workflow)
        self.assertNotIn("lw.PPOCR.C-browser-legacy", workflow)
        self.assertIn("name: lw.PPOCR.C-Windows-X64", workflow)
        self.assertIn("name: lw.PPOCR.C-Linux-X64", workflow)
        self.assertIn("name: lw.PPOCR.C-browser-${{ github.sha }}", workflow)
        self.assertIn("Require an annotated tag for stable releases", workflow)
